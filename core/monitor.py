...
class BlockMonitorDaemon:
    ...
    def handle_new_block_v2(self, block_identifiers: list[int]):
        ...
        txns: list[HexBytes] = [
            txn for blk in responses for txn in blk["transactions"]]
        txn_count = len(txns)
        print(f"\t{len(txns)} txns from No.{block_identifiers[0]}",
              f"to No.{block_identifiers[-1]}")
        if len(txns) <= 0:
    ...
    async def monitor_blocks(self):
        ...
        while True:
            try:
                ...
                print("--- lags: {} blocks, remote latest: {}".format(
                    current_block - latest_block, current_block))
                # limit batch size
                current_block = min(current_block, latest_block+2)
                print("\tprocessing {} blocks".format(
                    current_block - latest_block))
                self.handle_new_block_v2(
                    list(range(latest_block+1, current_block+1)))
                print()
